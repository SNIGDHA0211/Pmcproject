...
import { ContractPerformanceRecord, ContractValueRecord, User, UserRole, Project, ProjectStatus, DPR } from '../types';
...
const Dashboard: React.FC<DashboardProps> = ({ user, projects, dprs, projectDocuments = [], onViewProject, onReviewProjects, onStatClick, onSubmitDPR }) => {
  ...
  const [contractData, setContractData] = useState<ContractValueRecord | null>(null);
    ...
    const fetchContractData = async () => {
      ...
      try {
        const response = await contractValuesApi.getContractValues({
          projectName: leadProject.title,
          contractType: 'SCL',
        });
        const rows = unwrapList<any>(response.data);
        if (rows.length > 0) {
          setContractData(normalizeContractValueRecord(rows[0], leadProject.title, 'SCL'));
        } else {
          setContractData(null);
        }
      } catch (error) {
  ...
  useEffect(() => {
    const fetchContractPerformanceData = async () => {
      ...
      try {
        ...
        const rows = unwrapList<any>(response.data);
        if (rows.length > 0) {
          setContractPerformanceData(normalizeContractPerformanceRecord(rows[0]));
        } else {
          setContractPerformanceData(null);
        }
      } catch (error) {
          ...
          <div className="grid grid-cols-2 gap-4 flex-1">
            <div className={`p-5 border rounded-3xl transition-all cursor-default ${themeClasses.bgSecondary} ${themeClasses.border} ${themeClasses.bgHover} flex flex-col justify-between`}>
              ...
              <h4 className={`text-2xl font-black ${themeClasses.textPrimary}`}>
                {isLoadingContract ? '...' : formatINR(contractData?.revisedContractValue || leadProject.finances?.revisedValue || 0)}
              </h4>
            </div>
            <div className={`p-5 border rounded-3xl transition-all cursor-default ${themeClasses.bgSecondary} ${themeClasses.border} ${themeClasses.bgHover} flex flex-col justify-between`}>
              <p className={`text-[10px] font-black text-emerald-300 uppercase tracking-widest mb-2`}>Billed Value</p>
              <h4 className={`text-2xl font-black ${themeClasses.textPrimary}`}>
                {isLoadingContractPerformance ? '...' : formatINR(contractPerformanceData?.billedValue || 0)}
              </h4>
            </div>
            <div className={`p-5 border rounded-3xl transition-all cursor-default ${themeClasses.bgSecondary} ${themeClasses.border} ${themeClasses.bgHover} flex flex-col justify-between`}>
              <p className={`text-[10px] font-black uppercase tracking-widest ${themeClasses.textMuted} mb-2`}>Actual Receipt Value</p>
              <h4 className={`text-2xl font-black ${themeClasses.textPrimary}`}>
                {isLoadingContractPerformance ? '...' : formatINR(contractPerformanceData?.actualReceiptValue || 0)}
              </h4>
            </div>
            <div className={`p-5 border rounded-3xl transition-all cursor-default ${themeClasses.bgSecondary} ${themeClasses.border} ${themeClasses.bgHover} flex flex-col justify-between`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className={`lg:col-span-3 ${themeClasses.glassCard} p-8 rounded-[3rem] border ${themeClasses.border} flex flex-col items-center justify-between`}>
          ...
          <div className={`mt-8 text-center w-full p-5 border rounded-3xl ${themeClasses.bgSecondary} ${themeClasses.border}`}>
            ...
            <p className={`text-2xl font-black ${themeClasses.textPrimary}`}>{(leadProject.safety?.totalManhours ?? 0).toLocaleString()}</p>
          </div>
        </div>
      </div>
          ...
          <div className={`${isDarkTheme ? 'bg-indigo-950' : 'bg-indigo-900'} text-white p-6 rounded-[2.5rem] shadow-xl relative overflow-hidden group`}>
            <div className="relative z-10">
              ...
              <div className="flex items-center gap-6">
                <div>
                  <p className="text-3xl font-black text-white">{(leadProject.safety?.totalManhours ?? 0).toLocaleString()}</p>
        ...
        <div className={`lg:col-span-5 ${themeClasses.glassCard} p-8 rounded-[3rem] border ${themeClasses.border} flex flex-col`}>
          ...
          <div className="h-[250px] flex-1">
            {isLoadingManpower ? (
              ...
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={manpowerData.map((d: any) => ({ name: d.month_year, planned: d.planned_manpower || 0, actual: d.actual_manpower || 0 }))}>
                  <defs>
          ...
          <div className={`mt-8 pt-8 border-t ${themeClasses.border} flex justify-between items-center gap-4`}>
            {(() => {
              const latest = manpowerData.length > 0 ? manpowerData[manpowerData.length - 1] : null;
              return (
                <>
                  <div className="text-center flex-1">
                    <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${themeClasses.textMuted}`}>Planned</p>
                    <p className={`text-xl font-black ${themeClasses.textPrimary}`}>{latest?.planned_manpower || 0}</p>
                  </div>
                  ...
                  <div className="text-center flex-1">
                    ...
                    <p className={`text-xl font-black ${themeClasses.textPrimary}`}>{latest?.actual_manpower || 0}</p>
                  </div>
                  <div className={`w-px h-10 ${themeClasses.border}`}></div>
                  <div className="text-center flex-1">
                    <p className={`text-[10px] font-black uppercase tracking-widest mb-1 ${themeClasses.textMuted}`}>Efficiency</p>
                    <p className={`text-xl font-black ${themeClasses.textPrimary}`}>{latest?.manpower_efficiency ? `${(latest.manpower_efficiency * 100).toFixed(0)}%` : 'N/A'}</p>
                  </div>
                </>
              );
            })()}
