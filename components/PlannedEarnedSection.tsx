...
export const PlannedEarnedSectionBody: React.FC<{
  ...
  error?: string | null;
  emptyMessage?: string;
}> = ({ data, isLoading, error = null, emptyMessage = 'No planned vs actual data' }) => {
  ...
  const metrics = [
    ...
    },
    {
      ...
      icon: Activity,
      border: semanticBorderAccentClass(performancePercentage >= 100 ? 'positive' : performancePercentage >= 90 ? 'warning' : 'negative'),
      iconBg: semanticIconWrapClass(performancePercentage >= 100 ? 'positive' : performancePercentage >= 90 ? 'warning' : 'negative', isDarkTheme),
      valueClass: pctTone,
    },
  ] as const;
  ...
  return (
    <div className="flex flex-1 flex-col justify-center px-0 py-3 sm:px-1 sm:py-4">
      <div className="mb-3 flex items-start justify-between gap-2">
        <h4 className={`embedded-section-title min-w-0 break-words ${typo.embeddedSectionTitle}`}>{title}</h4>
      </div>
