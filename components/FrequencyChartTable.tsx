...
function fmt(v: unknown): string { return n(v).toLocaleString(); }
...
function compliance(conducted: number, required: number) {
  if (conducted >= required && required > 0) return { label: "Complied",    cls: "bg-emerald-100 text-emerald-800" } as const;
  if (conducted === 0)                       return { label: "Not Started", cls: "bg-slate-100  text-slate-700"   } as const;
  return                                            { label: "Shortfall",   cls: "bg-red-100    text-red-700"     } as const;
}
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            <tr className="bg-gradient-to-r from-indigo-600 to-purple-600 text-white">
              ...
              <th className={`${TH} text-right`}>Req. Tests</th>
          ...
          <tbody className={`divide-y ${isDarkTheme ? "divide-white/10" : "divide-slate-100"}`}>
            {normalised.map((row, i) => {
              const status = compliance(row.totalTestsConducted, row.requiredTestsUptoDate);
              return (
                <tr key={String(row.id ?? i)} className={`transition-colors ${isDarkTheme ? "hover:bg-white/5" : "hover:bg-slate-50"}`}>
                  ...
                  <td className="px-3 py-3 text-right font-bold text-indigo-600">{row.requiredTestsUptoDate}</td>
                  <td className="px-3 py-3 text-right font-bold text-emerald-600">{row.totalTestsConducted}</td>
                  <td className="px-3 py-3 text-center">
                    <span className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-[10px] font-bold ${status.cls}`}>
                      {status.label}
                    </span>
                  </td>
            ...
            <div key={String(row.id ?? i)} className={`p-4 ${isDarkTheme ? "bg-transparent" : "bg-white"}`}>
              ...
              <div className="flex items-start justify-between gap-2 mb-3">
                ...
                <div className="flex items-center gap-1 flex-shrink-0">
                  ...
                  >
                    <svg className={`h-4 w-4 transition-transform duration-200 ${isOpen ? "rotate-180" : ""}`}
              ...
              <div className="grid grid-cols-3 gap-2">
                {[
                  { label: "Required",  val: row.requiredTestsUptoDate, color: isDarkTheme ? "bg-indigo-950/60 text-indigo-300" : "bg-indigo-50 text-indigo-700" },
                  { label: "Conducted", val: row.totalTestsConducted,   color: isDarkTheme ? "bg-emerald-950/60 text-emerald-300" : "bg-emerald-50 text-emerald-700" },
                  { label: "Total Qty", val: fmt(row.totalQty),         color: isDarkTheme ? "bg-purple-950/60 text-purple-300" : "bg-purple-50 text-purple-700" },
                ].map(stat => (
              ...
              {isOpen && (
                <div className={`mt-3 pt-3 border-t space-y-3 ${isDarkTheme ? "border-white/10" : "border-slate-100"}`}>
                  ...
                  {(row.activityName || row.contractorName) && (
                    <div className="grid grid-cols-2 gap-3">
                      {row.activityName && (
                        <div>
                          <p className={`text-[9px] font-black uppercase tracking-wider mb-0.5 ${tc.textMuted}`}>Activity</p>
                          <p className={`text-sm font-semibold ${tc.textPrimary}`}>{String(row.activityName)}</p>
                        </div>
                      )}
                      {row.contractorName && (
                        <div>
                          <p className={`text-[9px] font-black uppercase tracking-wider mb-0.5 ${tc.textMuted}`}>Contractor</p>
                          <p className={`text-sm font-semibold ${tc.textPrimary}`}>{String(row.contractorName)}</p>
                        </div>
                      )}
                    </div>
                  )}
