...
function resolveProjectName(o: Record<string, unknown>, fallback?: string): string {
  const direct = strVal(o.project_name);
  if (direct) return direct;
  const project = o.project;
  if (typeof project === "string" && project.trim()) return project.trim();
  if (project && typeof project === "object" && !Array.isArray(project)) {
    const name = strVal((project as Record<string, unknown>).name);
    if (name) return name;
  }
  return fallback?.trim() || "";
}

function extractWeekEntries(o: Record<string, unknown>): WprWeekLike[] {
  for (const key of ["weeks", "weekly_reports", "weekly_data", "week_data"] as const) {
    const value = o[key];
    if (Array.isArray(value) && value.length > 0) {
      return value as WprWeekLike[];
    }
  }

  const hasWeekPayload =
    o.week != null ||
    o.week_number != null ||
    o.start_date != null ||
    o.date_from != null ||
    ...
    o.issues != null ||
    o.remarks != null ||
    o.quality_status != null ||
    o.activities != null;

  if (hasWeekPayload) {
    return [o as WprWeekLike];
  }

  return [];
}

function normalizeWprRecord(
  o: Record<string, unknown>,
  fallbackProjectName?: string
): WprRecordLike | null {
  const project_name = resolveProjectName(o, fallbackProjectName);
  if (!project_name) return null;

  return {
    id: o.id as string | number | undefined,
    project_name,
    period: typeof o.period === "string" ? o.period : strVal(o.month_year) || undefined,
    weeks: extractWeekEntries(o),
    project_summary: o.project_summary as WprRecordLike["project_summary"],
  };
}

function mergeWprRecordsByProject(records: WprRecordLike[]): WprRecordLike[] {
  const grouped = new Map<string, WprRecordLike>();

  for (const record of records) {
    const key = `${record.project_name}::${record.period || ""}`;
    const existing = grouped.get(key);
    if (!existing) {
      grouped.set(key, { ...record, weeks: [...record.weeks] });
      continue;
    }

    existing.weeks.push(...record.weeks);
    if (!existing.id && record.id) existing.id = record.id;
    if (!existing.period && record.period) existing.period = record.period;
    if (!existing.project_summary && record.project_summary) {
      existing.project_summary = record.project_summary;
    }
  }

  return Array.from(grouped.values()).map((record) => ({
    ...record,
    weeks: record.weeks.sort((a, b) => {
      const weekA = numFromUnknown(a.week ?? a.week_number);
      const weekB = numFromUnknown(b.week ?? b.week_number);
      if (weekA !== weekB) return weekA - weekB;
      const startA = strVal(a.start_date) || strVal(a.date_from);
      const startB = strVal(b.start_date) || strVal(b.date_from);
      return startA.localeCompare(startB);
    }),
  }));
}

/** Normalize GET /wpr/ payloads (list, envelope, or flat weekly rows). */
export function normalizeWprResponse(data: unknown, requestedProjectName?: string): WprRecordLike[] {
  if (data == null) return [];
  ...
  const fallback = requestedProjectName?.trim() || "";

  let records: WprRecordLike[] = [];
  if (list.length > 0) {
    records = list
      .map((item) =>
        item && typeof item === "object"
          ? normalizeWprRecord(item as Record<string, unknown>, fallback)
          : null
      )
      .filter((record): record is WprRecordLike => record != null);
  } else if (payload && typeof payload === "object" && !Array.isArray(payload)) {
    const single = normalizeWprRecord(payload as Record<string, unknown>, fallback);
    if (single) records = [single];
  }

  return mergeWprRecordsByProject(records);
}