...
const DrawingKpiCard: React.FC<DrawingKpiCardProps> = ({
  label,
  value,
  accent,
  icon,
  ...
  if ((variant === 'icon' || variant === 'compact') && icon) {
    const isCompact = variant === 'compact';
    if (isCompact) {
      return (
        <div
          className={`flex min-h-[5.5rem] flex-col rounded-lg border px-2.5 py-2.5 ${
        ...
        >
          <div className="flex min-w-0 flex-col gap-1">
            ...
            <p className={`${typo.metricKpiLabel} ${DASHBOARD_STATUS_METRIC_LABEL_CLASS(isDarkTheme)}`}>{label}</p>
          </div>
          <p
...
export default React.memo(DrawingKpiCard);
