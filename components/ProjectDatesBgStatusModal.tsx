...
import { Pencil, ShieldCheck, ShieldPlus, Trash2 } from 'lucide-react';
    ...
    return (
      <div className={sectionCls}>
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <p className={`flex items-center gap-2 text-xs font-black uppercase tracking-wide ${themeClasses.textPrimary}`}>
            <span className={`inline-flex h-5 w-5 items-center justify-center rounded text-[9px] font-bold ${badgeCls}`}>
              {badge}
            </span>
            {partyLabel}
          </p>
            ...
            <table className="w-full min-w-[520px] border-collapse">
              <thead>
                <tr className={`border-b ${isDarkTheme ? 'border-white/10' : 'border-slate-200'}`}>
                  ...
                  {party === 'CONTRACTOR' && scope.mode === 'all' && (
                    <th className={`pb-2 pr-3 ${tableHeadCls}`}>Contractor</th>
                  )}
              ...
              <tbody>
                {entries.map((entry) => (
                  ...
                  >
                    <td className={`pr-3 ${tableCellCls}`}>{entry.bg_name}</td>
                    {party === 'CONTRACTOR' && scope.mode === 'all' && (
                      <td className={`pr-3 ${tableCellCls}`}>
                        {entry.contractor_name || '—'}
                      </td>
                    )}
                    <td className={`pr-3 tabular-nums ${tableCellCls}`}>
  ...
  return (
    <ModalPortal open>
      <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
        ...
        >
          <div
            className={`flex shrink-0 items-start justify-between gap-4 border-b px-5 py-4 sm:px-6 ${themeClasses.border} ${isDarkTheme ? 'bg-blue-600/10' : 'bg-blue-50/70'}`}
          >
            <div className="flex min-w-0 items-center gap-3">
              <span
                className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${isDarkTheme ? 'bg-blue-500/20 text-blue-400 ring-1 ring-blue-500/30' : 'bg-blue-100 text-blue-600 ring-1 ring-blue-200'}`}
              >
                <ShieldCheck size={20} strokeWidth={2} />
              </span>
              <div className="min-w-0">
                <h3 className={`text-base font-bold sm:text-lg ${themeClasses.textPrimary}`}>
