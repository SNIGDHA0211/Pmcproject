...
/**
 * Optional Chrome / channel notification helpers.
 *
 * Day-to-day DPR & project emails are backend-driven: when FE calls create /
 * assign / submit / approve / reject APIs, the backend already queues email +
 * WebSocket. Prefer NOT calling these methods for those flows (avoids double emails).
 *
 * Never call /api/internal/dpr/executive-digest/ from the frontend.
 * Do not block UI on these calls — fire-and-forget only.
 */
export type DprApproverRole = 'Team Leader' | 'PMC Manager' | 'PMC Head';

export const notificationApi = {
  /** Unified optional endpoint POST /notifications/ch-notification/ */
  ...
  /** Specific endpoints (same payloads as unified). Optional / legacy only. */
  endpoints: {
    projectCreated: (projectId: string | number) =>
      api.post(API_ENDPOINTS.NOTIFICATIONS.PROJECT_CREATED, {
        project_id: projectId,
      }),
    teamLeadAssigned: (projectId: string | number, userId: number) =>
      api.post(API_ENDPOINTS.NOTIFICATIONS.TEAM_LEAD_ASSIGNED, {
        project_id: projectId,
        user_id: userId,
      }),
    siteEngineerAssigned: (projectId: string | number, userId: number) =>
      api.post(API_ENDPOINTS.NOTIFICATIONS.SITE_ENGINEER_ASSIGNED, {
        project_id: projectId,
        user_id: userId,
      }),
    dprSubmitted: (dprId: string | number) =>
      api.post(API_ENDPOINTS.NOTIFICATIONS.DPR_SUBMITTED, { dpr_id: dprId }),
    dprApproved: (dprId: string | number) =>
      api.post(API_ENDPOINTS.NOTIFICATIONS.DPR_APPROVED, { dpr_id: dprId }),
    dprRejected: (dprId: string | number) =>
      api.post(API_ENDPOINTS.NOTIFICATIONS.DPR_REJECTED, { dpr_id: dprId }),
  },

  /**
   * Fire-and-forget optional notify. Never await in UI-critical paths.
   * Prefer leaving this unused when backend already notifies on the mutation.
   */
  notifyOptionalSafe(
